...
class DataIngestion:
    ...
    def split_data_as_train_test(self) -> DataIngestionArtifact:
        try:
            ...
            us_ml_file_name = os.path.join(raw_data_dir , file_name)

            today_date = date.today()
            current_year = today_date.year

            us_ml_dataframe = pd.read_csv(us_ml_file_name)

            us_ml_dataframe[Target] = np.where(us_ml_dataframe[Target] == 'Denied' , 1 , 0)

            train_set = None
            test_set = None

            train_set , test_set = train_test_split(us_ml_dataframe , test_size=0.2 , random_state=42)
